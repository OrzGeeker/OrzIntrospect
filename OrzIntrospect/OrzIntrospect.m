...
#import "OrzIntrospect.h"
#import <dlfcn.h>
#import <WebKit/WebKit.h>
#import "OrzFrameView.h"
#import "OrzStatusBarOverlay.h"
...
@interface OrzIntrospect () <WKNavigationDelegate, OrzFrameViewDelegate, UITextViewDelegate>
...
OrzIntrospect *sharedInstance = nil;

@implementation OrzIntrospect
...
+ (void)defaultConfig {
    ...
    [OrzIntrospect sharedIntrospector].invokeGestureRecognizer = defaultGestureRecognizer;

#if TARGET_IPHONE_SIMULATOR
    [[OrzIntrospect sharedIntrospector] start];
...
+ (OrzIntrospect *)sharedIntrospector
{
#ifdef DEBUG
    if (!sharedInstance)
    {
        sharedInstance = [[OrzIntrospect alloc] init];
...
- (void)start
{
    ...
    if (!mainWindow)
    {
        NSLog(@"OrzIntrospect: Couldn't setup.  No main window?");
        return;
    }
    
    if (!self.statusBarOverlay)
    {
        self.statusBarOverlay = [[OrzStatusBarOverlay alloc] init];
    }
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(statusBarTapped) name:kOrzIntrospectNotificationStatusBarTapped object:nil];
    ...
    NSLog(@"OrzIntrospect is setup. %@ to start.", [kOrzIntrospectKeysInvoke isEqualToString:@" "] ? @"Push the space bar" : [NSString stringWithFormat:@"Type '%@'",  kOrzIntrospectKeysInvoke]);
}

- (void)takeFirstResponder
{
    if (![self.inputTextView becomeFirstResponder])
        NSLog(@"OrzIntrospect: Couldn't reclaim keyboard input.  Is the keyboard used elsewhere?");
}
...
- (void)setInvokeGestureRecognizer:(UIGestureRecognizer *)newGestureRecognizer
{
    ...
    _invokeGestureRecognizer = nil;
    _invokeGestureRecognizer = newGestureRecognizer;
    [_invokeGestureRecognizer addTarget:self action:@selector(invokeIntrospector)];
...
- (void)invokeIntrospector
{
    ...
    if (self.on)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kOrzIntrospectNotificationIntrospectionDidStart
                                                            object:nil];
    }
    else
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kOrzIntrospectNotificationIntrospectionDidEnd
                                                            object:nil];
    }
}
...
{
    if ([string isEqualToString:kOrzIntrospectKeysDisableForPeriod])
    {
        [self setKeyboardBindingsOn:NO];
        [[self inputTextView] resignFirstResponder];
        NSLog(@"OrzIntrospect: Disabled for %.1f seconds", kOrzIntrospectTemporaryDisableDuration);
        [self performSelector:@selector(setKeyboardBindingsOn:) withObject:[NSNumber numberWithFloat:YES] afterDelay:kOrzIntrospectTemporaryDisableDuration];
        return NO;
    }
    
    if ([string isEqualToString:kOrzIntrospectKeysInvoke])
    {
    ...
    if ([string isEqualToString:kOrzIntrospectKeysToggleViewOutlines])
    {
        [self toggleOutlines];
        return NO;
    }
    else if ([string isEqualToString:kOrzIntrospectKeysToggleNonOpaqueViews])
    {
        [self toggleNonOpaqueViews];
        return NO;
    }
    else if ([string isEqualToString:kOrzIntrospectKeysToggleFlashViewRedraws])
    {
        [self toggleRedrawFlashing];
        return NO;
    }
    else if ([string isEqualToString:kOrzIntrospectKeysToggleShowCoordinates])
    {
        [UIView animateWithDuration:0.15
                         ...
                         animations:^{
                             ...
                             if (self.showStatusBarOverlay)
                                 ...
                                 NSLog(@"OrzIntrospect: %@", coordinatesString);
                         }];
        return NO;
    }
    else if ([string isEqualToString:kOrzIntrospectKeysToggleHelp])
    {
        [self toggleHelp];
        return NO;
    }
    ...
    {
        if ([string isEqualToString:kOrzIntrospectKeysLogProperties])
        {
            [self logPropertiesForObject:self.currentView];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysLogAccessibilityProperties])
        {
            [self logAccessabilityPropertiesForObject:self.currentView];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysLogViewRecursive])
        {
            [self logRecursiveDescriptionForView:self.currentView];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysSetNeedsDisplay])
        {
            [self forceSetNeedsDisplay];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysSetNeedsLayout])
        {
            [self forceSetNeedsLayout];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysReloadData])
        {
            [self forceReloadOfView];
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysMoveUpInViewHierarchy])
        {
            ...
            {
                NSLog(@"OrzIntrospect: At top of view hierarchy.");
                return NO;
            }
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysMoveBackInViewHierarchy])
        {
            ...
            {
                NSLog(@"OrzIntrospect: At bottom of view history.");
                return NO;
            }
            
            [self selectView:[self.currentViewHistory objectAtIndex:indexOfCurrentView - 1]];
        }
        else if ([string isEqualToString:kOrzIntrospectKeysMoveDownToFirstSubview])
        {
            if (self.currentView.subviews.count>0) {
                [self selectView:[self.currentView.subviews objectAtIndex:0]];
            }else{
                NSLog(@"OrzIntrospect: No subviews.");
                return NO;
            }
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysMoveToNextSiblingView])
        {
            ...
            if (currentViewsIndex==NSNotFound) {
                NSLog(@"OrzIntrospect: BROKEN HIERARCHY.");
                ...
                NSLog(@"OrzIntrospect: No next sibling views.");
                return NO;
            }
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysMoveToPrevSiblingView])
        {
            NSUInteger currentViewsIndex = [self.currentView.superview.subviews indexOfObject:self.currentView];
            if (currentViewsIndex==NSNotFound) {
                NSLog(@"OrzIntrospect: BROKEN HIERARCHY.");
                ...
                NSLog(@"OrzIntrospect: No previous sibling views.");
            }
            return NO;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysLogCodeForCurrentViewChanges])
        {
        ...
        CGRect frame = self.currentView.frame;
        if ([string isEqualToString:kOrzIntrospectKeysNudgeViewLeft])
            frame.origin.x -= 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysNudgeViewRight])
            frame.origin.x += 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysNudgeViewUp])
            frame.origin.y -= 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysNudgeViewDown])
            frame.origin.y += 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysCenterInSuperview])
            frame = CGRectMake(floorf((self.currentView.superview.frame.size.width - frame.size.width) / 2.0f),
                               ...
                               frame.size.width,
                               frame.size.height);
        else if ([string isEqualToString:kOrzIntrospectKeysIncreaseWidth])
            frame.size.width += 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysDecreaseWidth])
            frame.size.width -= 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysIncreaseHeight])
            frame.size.height += 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysDecreaseHeight])
            frame.size.height -= 1.0f;
        else if ([string isEqualToString:kOrzIntrospectKeysIncreaseViewAlpha])
        {
            if (self.currentView.alpha < 1.0f)
                self.currentView.alpha += 0.05f;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysDecreaseViewAlpha])
        {
            if (self.currentView.alpha > 0.0f)
                self.currentView.alpha -= 0.05f;
        }
        else if ([string isEqualToString:kOrzIntrospectKeysEnterGDB])
        {
            UIView *view = self.currentView;
            view.tag = view.tag;	// suppress the xcode warning about an unused variable.
            NSLog(@"OrzIntrospect: access current view using local 'view' variable.");
            //            DEBUGGER;
            return NO;
        }
...
- (void)logCodeForCurrentViewChanges
{
    ...
    if (outputString.length == 0)
        NSLog(@"OrzIntrospect: No changes made to %@.", self.currentView.class);
    else
...
- (void)updateFrameView
{
    ...
    if (!self.frameView)
    {
        self.frameView = [[OrzFrameView alloc] initWithFrame:CGRectMake(0, 0, mainWindow.frame.size.width, mainWindow.frame.size.height) delegate:self];
...
- (void)updateStatusBar
{
    ...
    {
        self.statusBarOverlay.leftLabel.text = @"OrzIntrospect";
        self.statusBarOverlay.rightLabel.text = [NSString stringWithFormat:@"'%@' for help", kOrzIntrospectKeysToggleHelp];
    }
...
- (void)logRecursiveDescriptionForView:(UIView *)view
{
    ...
    NSLog(@"OrzIntrospect: %@", [view recursiveDescription]);
#endif
}
...
- (void)toggleNonOpaqueViews
{
    ...
    UIWindow *mainWindow = [self mainWindow];
    [self setBackgroundColor:(self.highlightNonOpaqueViews) ? kOrzIntrospectOpaqueColor : [UIColor clearColor]
...
- (void)flashRect:(CGRect)rect inView:(UIView *)view
{
    if (self.flashOnRedraw)
    {
        ...
        layer.frame = rect;
        layer.backgroundColor = kOrzIntrospectFlashOnRedrawColor.CGColor;
        [view.layer addSublayer:layer];
        [layer performSelector:@selector(removeFromSuperlayer) withObject:nil afterDelay:kOrzIntrospectFlashOnRedrawFlashLength];
    }
}
...
#pragma mark OrzIntrospector Help
...
- (void)toggleHelp
{
    ...
    if (self.showingHelp)
    {
        ...
        [helpString appendString:@"</style></head><body><h1>OrzIntrospect</h1>"];
        ...
        [helpString appendString:@"<p>GitHub project: <a href='https://github.com/domesticcatsoftware/OrzIntrospect'>github.com/domesticcatsoftware/OrzIntrospect/</a></p>"];
        
        [helpString appendString:@"<div class='bindings'><h1>Key Bindings</h1>"];
        [helpString appendString:@"<p>Edit OrzIntrospectSettings.h to change key bindings.</p>"];
        
        [helpString appendString:@"<h2>General</h2>"];
        
        [helpString appendFormat:@"<div><span class='name'>Invoke Introspector</span><div class='key'>%@</div></div>", ([kOrzIntrospectKeysInvoke isEqualToString:@" "]) ? @"spacebar" : kOrzIntrospectKeysInvoke];
        [helpString appendFormat:@"<div><span class='name'>Toggle View Outlines</span><div class='key'>%@</div></div>", kOrzIntrospectKeysToggleViewOutlines];
        [helpString appendFormat:@"<div><span class='name'>Toggle Highlighting Non-Opaque Views</span><div class='key'>%@</div></div>", kOrzIntrospectKeysToggleNonOpaqueViews];
        [helpString appendFormat:@"<div><span class='name'>Toggle Help</span><div class='key'>%@</div></div>", kOrzIntrospectKeysToggleHelp];
        [helpString appendFormat:@"<div><span class='name'>Toggle flash on <span class='code'>drawRect:</span> (see below)</span><div class='key'>%@</div></div>", kOrzIntrospectKeysToggleFlashViewRedraws];
        [helpString appendFormat:@"<div><span class='name'>Toggle coordinates</span><div class='key'>%@</div></div>", kOrzIntrospectKeysToggleShowCoordinates];
        [helpString appendString:@"<div class='spacer'></div>"];
        
        [helpString appendString:@"<h2>When a view is selected</h2>"];
        [helpString appendFormat:@"<div><span class='name'>Log Properties</span><div class='key'>%@</div></div>", kOrzIntrospectKeysLogProperties];
        [helpString appendFormat:@"<div><span class='name'>Log Accessibility Properties</span><div class='key'>%@</div></div>", kOrzIntrospectKeysLogAccessibilityProperties];
        [helpString appendFormat:@"<div><span class='name'>Log Recursive Description for View</span><div class='key'>%@</div></div>", kOrzIntrospectKeysLogViewRecursive];
        [helpString appendFormat:@"<div><span class='name'>Enter GDB</span><div class='key'>%@</div></div>", kOrzIntrospectKeysEnterGDB];
        [helpString appendFormat:@"<div><span class='name'>Move up in view hierarchy</span><div class='key'>%@</div></div>", ([kOrzIntrospectKeysMoveUpInViewHierarchy isEqualToString:@""]) ? @"page up" : kOrzIntrospectKeysMoveUpInViewHierarchy];
        [helpString appendFormat:@"<div><span class='name'>Move back down in view hierarchy</span><div class='key'>%@</div></div>", ([kOrzIntrospectKeysMoveBackInViewHierarchy isEqualToString:@""]) ? @"page down" : kOrzIntrospectKeysMoveBackInViewHierarchy];
        [helpString appendString:@"<div class='spacer'></div>"];
        
        [helpString appendFormat:@"<div><span class='name'>Nudge Left</span><div class='key'>\uE235 / %@</div></div>", kOrzIntrospectKeysNudgeViewLeft];
        [helpString appendFormat:@"<div><span class='name'>Nudge Right</span><div class='key'>\uE234 / %@</div></div>", kOrzIntrospectKeysNudgeViewRight];
        [helpString appendFormat:@"<div><span class='name'>Nudge Up</span><div class='key'>\uE232 / %@</div></div>", kOrzIntrospectKeysNudgeViewUp];
        [helpString appendFormat:@"<div><span class='name'>Nudge Down</span><div class='key'>\uE233 / %@</div></div>", kOrzIntrospectKeysNudgeViewDown];
        [helpString appendFormat:@"<div><span class='name'>Center in Superview</span><div class='key'>%@</div></div>", kOrzIntrospectKeysCenterInSuperview];
        [helpString appendFormat:@"<div><span class='name'>Increase Width</span><div class='key'>alt + \uE234 / %@</div></div>", kOrzIntrospectKeysIncreaseWidth];
        [helpString appendFormat:@"<div><span class='name'>Decrease Width</span><div class='key'>alt + \uE235 / %@</div></div>", kOrzIntrospectKeysDecreaseWidth];
        [helpString appendFormat:@"<div><span class='name'>Increase Height</span><div class='key'>alt + \uE233 / %@</div></div>", kOrzIntrospectKeysIncreaseHeight];
        [helpString appendFormat:@"<div><span class='name'>Decrease Height</span><div class='key'>alt + \uE232 / %@</div></div>", kOrzIntrospectKeysDecreaseHeight];
        [helpString appendFormat:@"<div><span class='name'>Increase Alpha</span><div class='key'>%@</div></div>", kOrzIntrospectKeysIncreaseViewAlpha];
        [helpString appendFormat:@"<div><span class='name'>Decrease Alpha</span><div class='key'>%@</div></div>", kOrzIntrospectKeysDecreaseViewAlpha];
        [helpString appendFormat:@"<div><span class='name'>Log view code</span><div class='key'>%@</div></div>", kOrzIntrospectKeysLogCodeForCurrentViewChanges];
        [helpString appendString:@"<div class='spacer'></div>"];
        
        [helpString appendFormat:@"<div><span class='name'>Call setNeedsDisplay</span><div class='key'>%@</div></div>", kOrzIntrospectKeysSetNeedsDisplay];
        [helpString appendFormat:@"<div><span class='name'>Call setNeedsLayout</span><div class='key'>%@</div></div>", kOrzIntrospectKeysSetNeedsLayout];
        [helpString appendFormat:@"<div><span class='name'>Call reloadData (UITableView only)</span><div class='key'>%@</div></div>", kOrzIntrospectKeysReloadData];
        [helpString appendString:@"</div>"];
        
        [helpString appendFormat:@"<h1>GDB</h1><p>Push <span class='code'>%@</span> (backtick) to jump into GDB.  The currently selected view will be available as a variable named 'view'.</p>", kOrzIntrospectKeysEnterGDB];
        
        [helpString appendFormat:@"<h1>Flash on <span class='code'>drawRect:</span> calls</h1><p>To implement, call <span class='code'>[[OrzIntrospect sharedIntrospector] flashRect:inView:]</span> inside the <span class='code'>drawRect:</span> method of any view you want to track.</p><p>When Flash on <span class='code'>drawRect:</span> is toggled on (binding: <span class='code'>%@</span>) the view will flash whenever <span class='code'>drawRect:</span> is called.</p>", kOrzIntrospectKeysToggleFlashViewRedraws];
        
        [helpString appendFormat:@"<h1>Naming objects & logging code</h1><p>By providing names for objects using <span class='code'>setName:forObject:accessedWithSelf:</span>, that name will be shown in the status bar instead of the class of the view.</p><p>This is also used when logging view code (binding: <span class='code'>%@</span>).  Logging view code prints formatted code to the console for properties that have been changed.</p><p>For example, if you resize/move a view using the nudge keys, logging the view code will print <span class='code'>view.frame = CGRectMake(50.0 ..etc);</span> to the console.  If a name is provided then <span class='code'>view</span> is replaced by the name.</p>", kOrzIntrospectKeysLogCodeForCurrentViewChanges];
        
        [helpString appendString:@"<h1>License</h1><p>OrzIntrospect is made available under the <a href='http://en.wikipedia.org/wiki/MIT_License'>MIT license</a>.</p>"];
...
-(void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    NSURL *reqestURL = navigationAction.request.URL;
    NSString *requestString = [reqestURL absoluteString];
    ...
    if ([requestString isEqualToString:@"about:blank"]) {
        ...
        [[UIApplication sharedApplication] openURL:reqestURL];
    }
}
...
- (void)logPropertiesForObject:(id)object
{
    ...
    [outputString appendString:@"\n"];
    NSLog(@"OrzIntrospect: %@", outputString);
}
